...
pub struct EntryMeta {
    ...
    pub footer_len: usize, // header[24..28]
}
...
pub fn decrypt_all(buf: &[u8], key: &[u8]) -> Result<Vec<DecryptedEntry>> {
    ensure_bnd4(buf)?;
...
pub fn find_ng(usr: &[u8], anchor_pattern: &[u8]) -> Option<(usize, u32)> {
...
pub fn set_ng(usr: &mut [u8], new_value: u32, anchor_pattern: &[u8]) -> Result<()> {
    if let Some((off, _)) = find_ng(usr, anchor_pattern) {
...
pub fn reencrypt_entry_into(sl2: &mut [u8], e: &DecryptedEntry, key: &[u8]) -> Result<()> {
    let meta = &e.meta;
    ...
    let ct_len = ct.len();
    let enc = Encryptor::<Aes128>::new_from_slices(key, &e.iv)?;
